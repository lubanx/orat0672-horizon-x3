...
int main(int argc, char *argv[])
{
	...
	{
		if (0 == strlen(sensor_lists[i].sensor_tag))
			break;
	...
	printf("**********************************************************\n");
	printf("*                                                        *\n");
	printf("*              this is tof camera test.                  *\n");
	printf("*                                                        *\n");
	printf("**********************************************************\n");

	char *cPtrpmdPath = "/app/";
	char *i2c_name = "/dev/i2c-2";
	DepthMapWrapperGetCalibrationData(i2c_name, cPtrpmdPath); // 获取标定文件，在线模式打开
	DepthMapWrapperSetUp(i2c_name, cPtrpmdPath, IRS2875C);
