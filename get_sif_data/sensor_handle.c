...
#define ORAT0762_WIDTH 240
#define ORAT0762_HEIGHT 1413
...
static int x3_dumpToFile_transfer(char *filename, char *srcBuf, unsigned int size)
{
	FILE *yuvFd = NULL;
	char *buffer = NULL;
	char raw12_with_no_stride[ORAT0762_WIDTH * ORAT0762_HEIGHT * 3 / 2];
	char raw16_with_no_stride[ORAT0762_WIDTH * ORAT0762_HEIGHT * 2];
	int byte_index = 0;
	int pixel_index = 0;
	unsigned short *buffer_16 = (unsigned short *)raw16_with_no_stride;
	/*
		yuvFd = fopen(filename, "w+");
	...
	*/
	...
	for (int i = 0; i < size / (ORAT0762_WIDTH * 3 / 2 + 8); i++)
	{
		for (int j = 0; j < ORAT0762_WIDTH * 3 / 2; j++)
			raw12_with_no_stride[i * ORAT0762_WIDTH * 3 / 2 + j] = buffer[i * (ORAT0762_WIDTH * 3 / 2 + 8) + j];
	}

	while (byte_index < (ORAT0762_WIDTH * ORAT0762_HEIGHT * 3 / 2 - 2))
	{
		buffer_16[pixel_index + 0] = (((unsigned short)(raw12_with_no_stride[byte_index + 0])) << 4) & 0xff0;
		buffer_16[pixel_index + 0] = buffer_16[pixel_index + 0] | (unsigned short)((raw12_with_no_stride[byte_index + 2] >> 0) & 0x000f);

		buffer_16[pixel_index + 1] = (((unsigned short)(raw12_with_no_stride[byte_index + 1])) << 4) & 0xff0;
		buffer_16[pixel_index + 1] = buffer_16[pixel_index + 1] | (unsigned short)((raw12_with_no_stride[byte_index + 2] >> 4) & 0x000f);
	...
	//	fwrite(raw16_with_no_stride, 1, ORAT0762_WIDTH*ORAT0762_HEIGHT*2, yuvFd);
	...
	printf("filedump(%s, size(%d) is successed\n", filename, ORAT0762_WIDTH * ORAT0762_HEIGHT * 2);
...
int sif_dump_func(void)
{
	...
	char file_name_raw12_no_stride[128] = {0};
	char file_name_raw16_no_stride[128] = {0};
	...
	else
	{
		...
		for (int i = 0; i < dump_info.raw.plane_count; i++)
		{
			if (sif_raw->img_info.img_format == 0)
			{
				...
				sprintf(file_name_raw12_no_stride, "pipe%d_plane%d_%ux%u_frame_%03d_raw12_no_stride.raw",
						dump_info.ctx_id,
						i,
						dump_info.raw.xres[i],
						dump_info.raw.yres[i],
						dump_info.raw.frame_id);
				sprintf(file_name_raw16_no_stride, "pipe%d_plane%d_%ux%u_frame_%03d_raw16_no_stride.raw",
						dump_info.ctx_id,
						i,
						dump_info.raw.xres[i],
						dump_info.raw.yres[i],
				...
				x3_dumpToFile_transfer(file_name_raw16_no_stride, dump_info.raw.addr[i], dump_info.raw.size[i]);
				gettimeofday(&time_next, NULL);
				int time_cost = time_cost_ms(&time_now, &time_next);
