...
#define SENSOR_TAG "ORAT0672"

#define F_WIDTH  240
#define F_HEIGHT 1413

MIPI_SENSOR_INFO_S SENSOR_2LANE_ORAT0672_10FPS_12BIT_LINEAR_INFO =
{
	...
	.sensorInfo = {
		...
		.sensor_addr = 0x3d, // sensor i2c 设备地址
		...
		.sensor_name = "orat0672", // sensor的名字，在libcam.so中会根据这个名字组合出 libf37.so
		//.gpio_num = 1,
		//.gpio_pin = 111,//119,  //19
		//.gpio_level = 0,
		.gpio_num = 8,
		.gpio_pin = {-1,-1,-1,-1,-1,-1,111,-1},//111,//119,  //19
		.gpio_level = {0,0,0,0,0,0,1,0},//0,
		.power_mode = 1,
...
MIPI_ATTR_S MIPI_2LANE_SENSOR_ORAT0672_10FPS_12BIT_LINEAR_ATTR =
{
	.mipi_host_cfg = {
		.lane = 2, // 硬件上sensor用了几个mipi数据lane，f37用 1 lane, imx415 4K 用 4 lane
		.datatype = 0x2c, // sensor的输出数据类型，请参考 https://developer.horizon.ai/documents/mpp_develop/3-Video_Input_zh_CN/3-Video_Input_zh_CN.html#data-type
		...
		.height = F_HEIGHT,
		.linelenth = 2496,//928,//2496, // sensor 输出带 blanking 的总行长， 一般根据sensor供应商的配置参数配置就行
		.framelenth = 13080,//5000,//13080, // sensor 输出带 blanking的总行数
		.settle = 45,//30,//45, // sensor 输出实际Ttx-zero + Ttx-prepare时间（ clk 为单位）, 一般0-100之间尝试
		.channel_num = 4,//1, // 使用虚通道的个数, linear 用一个，dol2 用 2 个， dol3 用3个
		.channel_sel = {0, 1, 2, 3}//{0} // 保存每个虚通道的值，在调用HB_VIN_SetDevVCNumber接口时用第一个，调用HB_VIN_AddDevVCNumber用后面的2 3 4的配置
	},
	.dev_enable = 0 // mipi dev 是否使能， 1是使能， 0 是关闭, 只用开启mipi bypass的开启
};


VIN_DEV_ATTR_S DEV_ATTR_ORAT0672_LINEAR_BASE = {
	.stSize = { // 输入的数据
	...
	.outDdrAttr = { // sif(dev) 的输出到 ddr 配置
		.stride = 320,//F_WIDTH*1.5+8, // 硬件 stride 跟格式匹配，通过行像素根据raw数据bit位数计算得来（而且结果要32对+齐），8bit：x1, 10bit: x1.25 12bit: x1.5,例F37 raw10，1920 x 1.25 = 2400
...
VIN_PIPE_ATTR_S PIPE_ATTR_ORAT0672_LINEAR_BASE = {
...
static int set_sensor_param(void)
{
	...
	/*定义 sensor   初始化的属性信息 */
	snsinfo = SENSOR_2LANE_ORAT0672_10FPS_12BIT_LINEAR_INFO;
	/*定义 mipi 初始化参数信息 */
	mipi_attr = MIPI_2LANE_SENSOR_ORAT0672_10FPS_12BIT_LINEAR_ATTR;
	/*定义 dev 初始化的属性信息 */
	devinfo = DEV_ATTR_ORAT0672_LINEAR_BASE;
	/*定义 pipe 属性信息 */
	pipeinfo = PIPE_ATTR_ORAT0672_LINEAR_BASE;
